...
from datasets_fault.SequenceDatasets import*
...
def get_files(root, N):
    ...
    for i in range(len(N)):
        ...
        for i in tqdm(range(len(HBdata))):
            for w1 in range(20):
                name1 = state+"_"+HBdata[i]+"_"+str(w1+10)
        ...
        for j in tqdm(range(len(ADBdata))):
            for w2 in range(20):
                name2 = state+"_"+ADBdata[j]+"_"+str(w2+10)
        ...
        for k in tqdm(range(len(RDBdata))):
            for w3 in range(1):
                name3 = state+"_"+RDBdata[k]+"_"+str(w3+10)
...
class PU(object):
    ...
    def data_split(self, transfer_learning=True):
        if transfer_learning:
            ...
            return source_train, target_train, target_val, target_noshuffle
        else:
            #get source train and val
            list_data = get_files(self.data_dir, self.source_N)
            data_pd = pd.DataFrame({"data": list_data[0], "label": list_data[1]})
